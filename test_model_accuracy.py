#!/usr/bin/env python3
"""
Test ML Model Accuracy on Historical NFL Data
This script evaluates the betting model's performance on past games to determine if it's profitable.
"""
...
# Database connection
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://postgres:QAmpFszazifVixDGzdvWNXJTdzoXFgYw@maglev.proxy.rlwy.net:48520/railway")
...
# Get completed games from the 2024 season
print("📅 Fetching completed games from 2024 season...")
query = text("""
    SELECT
        game_id,
        game_date,
...
if len(results_df) == 0:
    print("❌ No predictions could be made. Model may be having issues.")
    sys.exit(1)

print(f"✅ Successfully predicted {len(results_df)} games")
print()
print("=" * 80)
...
print("-" * 80)
recent_games = results_df.head(10)
for idx, game in recent_games.iterrows():
    status = "✅" if game['correct'] else "❌"
    print(f"{status} Week {game['week']:2}: {game['matchup']:30} | Predicted: {game['predicted_winner']:3} ({game['confidence']*100:.1f}%) | Actual: {game['actual_winner']:3} ({game['home_score']}-{game['away_score']})")
...
if len(high_conf_games) > 0:
    ...
    if high_conf_accuracy > 60:
        ...
        print(f"⚠️  CAUTION: Even high confidence picks only hit {high_conf_accuracy:.1f}%")
...
if len(med_conf_games) > 0:
    ...
    if med_conf_accuracy > breakeven_rate:
        print(f"⚠️  CONSIDER: Medium confidence (58-65%) bets")
...
if overall_accuracy > 55:
    print("✅ Model shows promise - consider using for betting")
    print("💡 Stick to high confidence picks (65%+) for best results")
elif overall_accuracy > breakeven_rate:
    print("⚠️  Model is slightly profitable but marginal")
    print("💡 Only bet high confidence picks and use small stakes")
else:
    print("❌ Model is NOT profitable - DO NOT USE FOR BETTING")
    print("💡 Model needs more training data or feature engineering")
